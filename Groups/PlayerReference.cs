...
{
	public static PlayerReference fromPlayerId(long id) => ZNet.instance.m_players.Where(p => p.m_characterID.UserKey == id).Select(fromPlayerInfo).FirstOrDefault();
	public static PlayerReference fromPlayerInfo(ZNet.PlayerInfo playerInfo) => new() { peerId = playerInfo.m_characterID.UserKey, name = playerInfo.m_name ?? "" };
